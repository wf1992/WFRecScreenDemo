...
//	self.backgroundColor = IS_IPAD ? [UIColor clearColor] : [UIColor colorWithRed:0.225 green:0.225 blue:0.225 alpha:1.000];
    self.backgroundColor=KSetColor(231, 231, 231);
    ...
    KZColorCompareView *colorView = [[KZColorCompareView alloc] initWithFrame:CGRectMake(10, 10, 36, 36)];
    ...
    */
    ...
    CGFloat red=[[[NSUserDefaults standardUserDefaults]objectForKey:@"selectedColorR"] floatValue];
    CGFloat green=[[[NSUserDefaults standardUserDefaults]objectForKey:@"selectedColorG"] floatValue];
    CGFloat blue=[[[NSUserDefaults standardUserDefaults]objectForKey:@"selectedColorB"] floatValue];
    CGFloat ap=[[[NSUserDefaults standardUserDefaults]objectForKey:@"selectedColorA"] floatValue];
    if (red!=0||green!=0||blue!=0||ap!=0) {
        self.selectedColor=[UIColor colorWithRed:red green:green blue:blue alpha:ap];
    }
    
    
    NYSliderPopover *widthSlider=[[NYSliderPopover alloc]initWithFrame:CGRectMake(30, self.frame.size.height-100, self.frame.size.width-30-20-60, 100)];
    ...
    [widthSlider setThumbTintColor:[UIColor whiteColor]];
    widthSlider.value=2.0;
    if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"selectedWidth"] intValue]>0) {
        widthSlider.value=[[[NSUserDefaults standardUserDefaults]objectForKey:@"selectedWidth"] intValue];
        widthSlider.minimumTrackTintColor=selectedColor;
        [widthSlider setThumbTintColor:selectedColor];
    }
    ...
    UIButton *sureBtn=[[UIButton alloc]initWithFrame:CGRectMake(self.frame.size.width-60, self.frame.size.height-50-18, 36, 36)];
    CALayer *layer=[sureBtn layer];
    layer.cornerRadius=3.0;
    layer.masksToBounds=YES;
    [sureBtn setBackgroundColor:KSetColor(255, 136, 45)];
    [sureBtn setTitle:KSetString(@"local_sure") forState:UIControlStateNormal];
    [sureBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [sureBtn.titleLabel setFont:[UIFont systemFontOfSize:11]];
...
{
    [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%f",selectedColor.red] forKey:@"selectedColorR"];
    [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%f",selectedColor.green] forKey:@"selectedColorG"];
    [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%f",selectedColor.blue] forKey:@"selectedColorB"];
    [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%f",selectedColor.alpha] forKey:@"selectedColorA"];
    [[NSUserDefaults standardUserDefaults]setObject:[NSString stringWithFormat:@"%d",(int)self.widthSlider.value] forKey:@"selectedWidth"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    [self removeFromSuperview];
    if ([self.delegate respondsToSelector:@selector(saveColor:width:)]) {
        [self.delegate saveColor:selectedColor width:(int)self.widthSlider.value];
    }
}
...
- (void) setSelectedColor:(UIColor *)color animated:(BOOL)animated
{
    ...
    [self.widthSlider setThumbTintColor:selectedColor];
    self.widthSlider.minimumTrackTintColor=selectedColor;
}
...
- (void) fixLocations
{
    ...
    if(self.bounds.size.width < self.bounds.size.height)
    {
        ...
        CGFloat swatchCellWidth = totalWidth / 4.0;
